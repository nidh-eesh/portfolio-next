                    ...
                    <div className="flex-1 flex items-center xl:justify-end order-1 xl:order-none mb-8 xl:mb-0">
                        <ul className="flex flex-col gap-10">
                            {info.map((item, index) => {
                                ...
                                } else if (item.title.toLowerCase().includes("phone") || item.title.toLowerCase().includes("mobile")) {
                                    href = `tel:${item.data.replace(/\s+/g, "")}`;
                                } else if (item.title.toLowerCase().includes("location") || item.title.toLowerCase().includes("address") || item.title.toLowerCase().includes("map")) {
                                    href = "https://maps.app.goo.gl/sHwE288iHfK4MD9s7";
                                }
                                ...
                                const content = (
                                    <div className="flex items-center gap-6 group">
                                        <div className="w-[52px] h-[52px] xl:w-[72px] xl:h-[72px] bg-[#27272c] text-accent rounded-md flex items-center justify-center group-hover:brightness-125 transition">
                                            <div className="text-[28px]">{item.icon}</div>
                                        </div>
                                        <div className="flex-1">
                                            <p className="text-white/60">{item.title}</p>
                                            <h3 className="text-xl group-hover:text-accent group-hover:underline">{item.data}</h3>
                                        </div>
                                    </div>
                                );
